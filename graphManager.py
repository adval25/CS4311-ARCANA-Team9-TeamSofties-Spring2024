...
def createGraphElements(dictOfNodes):
    graphElementList = []
...
def createGraph(selectedProject):
    vectorIdPositions = unconnectedNodePositions(getUniuqeVectorIds(selectedProject))
    dictOfNodes = eventsToNodes(selectedProject,vectorIdPositions)
    edgeCreator(dictOfNodes)
